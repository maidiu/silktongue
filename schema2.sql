...
(
    ...
    CONSTRAINT quiz_attempts_level_check CHECK (level >= 1 AND level <= 4)
)
...
(
    ...
    CONSTRAINT quiz_materials_word_id_fkey FOREIGN KEY (word_id)
        ...
        ON UPDATE NO ACTION
        ON DELETE CASCADE,
    CONSTRAINT quiz_materials_level_check CHECK (level >= 1 AND level <= 5),
    CONSTRAINT quiz_materials_question_type_check CHECK (question_type = ANY (ARRAY['spelling'::text, 'typing'::text, 'definition'::text, 'synonym'::text, 'antonym'::text, 'story'::text]))
)
