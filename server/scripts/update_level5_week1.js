...
async function updateLevel5Week1() {
  try {
    const filePath = path.join(__dirname, '../../weekly_quizzes/level5_quizzes_fixed.json');
    const quizzes = JSON.parse(fs.readFileSync(filePath, 'utf8'));
    
    console.log(`Updating Level 5 quizzes for ${quizzes.length} words...\n`);
    ...
    for (const quiz of quizzes) {
      ...
      // Delete existing Level 5 quiz
      await pool.query('DELETE FROM quiz_materials WHERE word_id = $1 AND level = 5', [word_id]);
      ...
      await pool.query(
        ...
        [
          ...
          quiz.reward_amount || 25
        ]
      );
      
      console.log(`✓ Updated ${quiz.word} Level 5`);
    }
    
    console.log(`\n✅ All Level 5 quizzes updated!`);
