...
async function updateLevel6Week1() {
  try {
    const filePath = path.join(__dirname, '../../weekly_quizzes/level6_quizzes.json');
    const quizzes = JSON.parse(fs.readFileSync(filePath, 'utf8'));
    
    console.log(`Updating Level 6 quizzes for ${quizzes.length} words...\n`);
